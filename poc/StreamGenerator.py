...
class StreamGenerator:
    ...
    def __init__(
        ...
        incremental=False,
        y_flip=0.01,
        difficulty_n_drifts=2,
        difficulty_concept_sigmoid_spacing=None,
        ...
        **kwargs,
        ...
        self.difficulty_n_drifts = difficulty_n_drifts
        self.difficulty_concept_sigmoid_spacing = difficulty_concept_sigmoid_spacing
        self.difficulty_amplitude = difficulty_amplitude
    ...
    def _difficulty_sigmoid(self, difficulty_sigmoid_spacing, difficulty_n_drifts):
        period = (
            int(np.ceil((self.n_chunks) / (difficulty_n_drifts)) if difficulty_n_drifts > 0 else int(self.n_chunks))
        )
        css = difficulty_sigmoid_spacing if difficulty_sigmoid_spacing is not None else 9999
        _probabilities = (
            logistic.cdf(
                np.concatenate(
                    [
                        ...
                        for i in range(difficulty_n_drifts)
                    ]
                )
            )
    ...
    def _make_classification(self):
        ...
        if self.difficulty_n_drifts > 0:
            # Get period and probabilities
            self.difficulty_probabilities = 1. - self._difficulty_sigmoid(
                self.difficulty_concept_sigmoid_spacing, self.difficulty_n_drifts
            )
            # print(self.difficulty_probabilities)
            # exit()
