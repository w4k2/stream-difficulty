...
for mode_id, mode in enumerate(modes):
    ...
    plt.tight_layout()            
    plt.savefig('fig/m_acc_%s.png' % mode)
...
for mode_id, mode in enumerate(modes):
    ...
    plt.tight_layout()                          
    plt.savefig('fig/m_time_%s.png' % mode)
...
for mode_id, mode in enumerate(modes):
    ...
    for n_c_id, n_c in enumerate(n_cycles):
        for c_id, c in enumerate(chunk_size):
            ...
            ax[n_c_id, c_id].scatter(np.arange(n_chunks), temp, c='cornflowerblue', alpha=0.5)

    plt.tight_layout()            
    plt.savefig('fig/m_sel_%s.png' % mode)
...
for mode_id, mode in enumerate(modes):
    ...
    plt.tight_layout()                             
    plt.savefig('fig/acc_%s.png' % mode)
...
for mode_id, mode in enumerate(modes):
    ...
    plt.tight_layout()                               
    plt.savefig('fig/time_%s.png' % mode)
...
for mode_id, mode in enumerate(modes):
    ...
    plt.tight_layout()            
    plt.savefig('fig/sel_%s.png' % mode)
